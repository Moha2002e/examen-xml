...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  <!-- Feuille XSLT « Pro » avec pagination. Elle reprend la table minimale en ajoutant
       quelques améliorations ergonomiques : les lignes contenant une
       localisation « loc right » sont mises en évidence et les listes
       déroulantes permettent de masquer/afficher les labels et localisations.
  ...
  <xsl:template match="/images">
    <html>
      <head>
        <meta charset="UTF-8"/>
        <title>PadChest – Images (Pro avec pagination)</title>
        <style>
          ...
          .loc-right { background-color: #ffe6e6; }
      ...
      <body>
        <h1>Images radiographiques – Version Pro</h1>
        ...
        <table>
          ...
          <xsl:for-each select="image">
            ...
            <tr class="image-row" data-page="{(position() - 1) idiv $itemsPerPage + 1}">
              ...
              <td>
                <details>
                  <summary><xsl:value-of select="count(labels/label)"/> label(s)</summary>
                  <ul>
              ...
              <td>
                <details>
                  <summary><xsl:value-of select="count(localizations/localization)"/> localisation(s)</summary>
                  <ul>
                    <xsl:for-each select="localizations/localization">
                      <li><xsl:value-of select="."/></li>
                    </xsl:for-each>
                    <xsl:if test="not(localizations/localization)">
                      <li>Aucune localisation</li>
                    </xsl:if>
                  </ul>
                </details>
              </td>
